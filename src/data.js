...
export async function getData(coords) {
  ...
  try {
    // Fetch current data
    const dayResponse = await fetch(`https://pro.openweathermap.org/data/2.5/weather?${coords}&units=${UNITS}&appid=${API_KEY}`, 
      { mode: 'cors' }
    );
    const dayData = await dayResponse.json();

    // Assign data to DOM
    document.getElementById('location').innerText = `${dayData.name}${(state) ? ", " + state : " (Current Location)"}`;
    ...
    const hourResponse = await fetch(`https://pro.openweathermap.org/data/2.5/forecast/hourly?${coords}&cnt=5&units=${UNITS}&appid=${API_KEY}`, 
      { mode: 'cors' }
    );
    ...
    for (let i = 0; i < hourArray.length; i++) {
      ...
      document.getElementById(`hour${i+1}`).innerText = readableDate.replace(" AM", " am").replace(" PM", " pm");
      document.getElementById(`hour${i+1}img`).src = imageMap[`${hourArray[i].weather[0].icon}.svg`];
      document.getElementById(`hour${i+1}temp`).innerText = Math.round(hourArray[i].main.temp) + "˚";
    }

    // Fetch 10-Day forecast data
    const weekResponse = await fetch(`https://pro.openweathermap.org/data/2.5/forecast/daily?${coords}&cnt=10&units=imperial&appid=${API_KEY}`,
      { mode: 'cors' }
    );
    ...
    for (let i = 0; i < weekArray.length; i++) {
      ...
      document.getElementById(`weekTemp${i+1}`).innerText = Math.round(weekArray[i].temp.day) + "˚";
      document.getElementById(`weekImg${i+1}`).src = imageMap[`${weekArray[i].weather[0].icon}.svg`];
      document.getElementById(`weekDesc${i+1}`).innerText = weekArray[i].weather[0].main;
      document.getElementById(`weekRange${i+1}`).innerText = "L:" + Math.round(weekArray[i].temp.min) + "˚" + " H:" + Math.round(weekArray[i].temp.max) + "˚";
    }
